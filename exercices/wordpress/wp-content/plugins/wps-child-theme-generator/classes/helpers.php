...
";
			...
			if ( isset( $_POST['be7ctg_send'] ) && isset( $_POST['be7ctg_send_email'] ) && $_POST['be7ctg_send_email'] != '' ) {
				if ( $zip === true ) {
					...
					$attachments = array( WP_CONTENT_DIR . '/themes/' . $new_themename . '.zip' );
					wp_mail( esc_attr( $_POST['be7ctg_send_email'] ), $subject, $message, '', $attachments );
				}
			}

			$zip_link = self::abs_path_to_url( WP_CONTENT_DIR . '/themes/' . $new_themename . '.zip' );
	...
	public static function _create_zip( $source, $file_name ) {
		...
		$zip = new \ZipArchive();
		$zip->open( WP_CONTENT_DIR . '/themes/' . $file_name . '.zip', \ZipArchive::CREATE | \ZipArchive::OVERWRITE );
